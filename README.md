...
```bash
docker run --rm --network=host -v privhome:/home/coder ghcr.io/smerschjohann/code-server-container/code-server:latest --port=8154 --auth=none
```

## with docker usage

```bash
docker run --rm --network=host -v privhome:/home/coder -v /var/run/docker.sock:/var/run/docker.sock ghcr.io/smerschjohann/code-server-container/code-server:latest --port=8154 --auth=none
        ...
        - name: PASSWORD
          valueFrom:
            secretKeyRef:
              key: password
              name: code-server
        image: ghcr.io/smerschjohann/code-server-container/code-server:latest
        imagePullPolicy: Always
        livenessProbe:
          failureThreshold: 3
          httpGet:
